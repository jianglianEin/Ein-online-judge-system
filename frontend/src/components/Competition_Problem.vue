...
export default {
    ...
    methods: {
        ...
        showDetailedProblem(){
            var id = this.$route.params.problemId;
        ...
        showSolutionOfCompetitionByProblemAndLanguageType(mode){
            var languageType = "";
            if(mode == "javascript"){
                languageType = "js";
            }else if(mode == "cmake"){
            ...
            var problemId = this.$route.params.problemId;
            var competitionId = this.$route.params.competitionId;
            var url = this.Competition + '/showSolutionOfCompetitionByProblemAndLanguageType';
            this.$http({
              method:'get',
              url: url,
              params: {
                ...
                languageType:languageType
                  ...
                  if(res.data.success){
                    ...
                    if(res.data.message=="new answer"){
                        this.codes[this.mode] = this.codes[this.mode]
                        this.commitCode = this.codes[this.mode]
                        console.log("success new answer")
                    }else{
                        const message = JSON.parse(res.data.message);
                        console.log(message)
                        this.codes[this.mode] = message.solutionOfCompetition.code;
                        this.commitCode = message.solutionOfCompetition.code;
                        console.log("success")
                    }
                    // this.discussionMsg = message;
                  }else{
        ...
        formdata.append('competitionId',this.$route.params.competitionId);
        formdata.append('problemId',this.$route.params.problemId);
        ...
        var url = this.Competition + '/commit';
        this.$http({
        url: url,
        method: 'post',
        data: formdata,
        headers: { 'Content-Type': 'multipart/form-data' }
                ...
                const message = JSON.parse(res.data.message);
                console.log(message)
                this.$router.push("/solutionOfCompetition/"+message.id);
    ...
    created() {
      ...
      this.showSolutionOfCompetitionByProblemAndLanguageType(this.mode);
    },

    watch: {
        mode(newMode,oldMode){
            this.showSolutionOfCompetitionByProblemAndLanguageType(newMode);
        }
    },
}
</script>
